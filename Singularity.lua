...
-- парсим JSON → таблица
local menuData = HttpService:JSONDecode(menuJSON)
...
-- грузим скрипты через loadstring (эксплойт)
for name, url in pairs(scriptsList) do
	...
	if success and scriptContent and scriptContent ~= "" then
		...
		if not ran then
			warn("Ошибка в " .. name .. ": " .. tostring(err))
		end
	else
		warn("Не удалось загрузить " .. name .. " с " .. url)
	end
end
